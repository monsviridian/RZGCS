...
def main():
    ...
    # Methode umbenennen, um Signalkonflikte zu vermeiden
    if hasattr(drone_view_model, 'connect') and not hasattr(drone_view_model, 'connectToDrone'):
        # Originalmethode speichern und umbenennen
        drone_view_model.connectToDrone = drone_view_model.connect
    ...
    # QML-Datei laden, falls vorhanden
    qml_file = os.path.join(os.path.dirname(os.getcwd()), "RZGCSContent", "App.qml")
    if os.path.exists(qml_file):
        print(f"Loading QML file: {qml_file}")
        engine.load(QUrl.fromLocalFile(qml_file))
    else:
        # Minimales Testfenster erstellen
        drone_view_model._logger.addLog("[INFO] RZGCS with MAVSDK MVVM integration started")
