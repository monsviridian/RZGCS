...
class RZGCSBackend(QObject):
    ...
    def __init__(self):
        ...
        # SITL-ViewModel für die Software-in-the-Loop Simulation
        self.sitl_view_model = SITLViewModel(self.logger)
        self.mission_view_model = MissionViewModel(self.logger)
        self.camera_view_model = CameraViewModel(self.logger)
        self.mission_planner_style = MissionPlannerStyle()
        
        # Firmware-Manager für die Firmware-Installation
        self.firmware_manager = FirmwareManager(self)
        
        # Property für QML-Zugriff
        # Hinweis: In PySide6 werden Signale direkt als Funktion aufgerufen, nicht mit .emit()
...
def main():
    ...
    # QML Debug Logger aktivieren
    print("Aktiviere QML Debug Logging...")
    engine.setOutputWarningsToStandardError(True)
    
    # Typen für QML registrieren
    qmlRegisterType(SensorViewModel, "RZGCS", 1, 0, "SensorViewModel")
    
    # Backend erstellen (behält die ursprüngliche Initialisierung)
    backend = RZGCSBackend()
    
    # Debug-Information über das MissionPlannerStyle-Objekt ausgeben
    print("DEBUG: MissionPlannerStyle Objekt verfügbar:", backend.mission_planner_style)
    
    # Typen für QML setzen
    print("Registriere Context Properties für QML...")
    ...
    engine.rootContext().setContextProperty("sitlViewModel", backend.sitl_view_model)
    
    # MissionPlannerStyle registrieren und Signale debuggen
    print("Registriere MissionPlannerStyle als missionPlannerStyle...")
    mps = backend.mission_planner_style
    engine.rootContext().setContextProperty("missionPlannerStyle", mps)
    
    # Debug-Informationen zu MissionPlannerStyle ausgeben
    print("MissionPlannerStyle Methoden:")
    methods = [method_name for method_name in dir(mps) if callable(getattr(mps, method_name)) and not method_name.startswith("_")]
    for method in methods:
        print(f"  - {method}")
    
    print("MissionPlannerStyle Signale:")
    signals = [attr for attr in dir(mps) if isinstance(getattr(mps.__class__, attr, None), Signal)]
    for signal in signals:
        print(f"  - {signal}")
        
    print("MissionPlannerStyle Properties:")
    properties = [prop for prop in dir(mps.__class__) if isinstance(getattr(mps.__class__, prop, None), Property)]
    for prop in properties:
        print(f"  - {prop}: {getattr(mps, prop, 'N/A')}")
    ...
    # SITL-ViewModel für Software-in-the-Loop Simulation registrieren
    engine.rootContext().setContextProperty("sitlViewModel", backend.sitl_view_model)
    print("SITL-ViewModel registriert!")
    
    # Debug MissionPlannerStyle setup
    print("MissionPlannerStyle instance:", backend.mission_planner_style)
    print("MissionPlannerStyle methods:", [method for method in dir(backend.mission_planner_style) if not method.startswith('_')])
    print("MissionPlannerStyle signals:", [attr for attr in dir(backend.mission_planner_style) if isinstance(getattr(backend.mission_planner_style, attr), Signal)])
    
    # Firmware-Manager für QML registrieren
    engine.rootContext().setContextProperty("firmwareManager", backend.firmware_manager)
    print("Firmware-Manager registriert!")
