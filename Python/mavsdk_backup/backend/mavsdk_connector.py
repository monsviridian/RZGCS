...
# Kompatibilität mit verschiedenen MAVSDK-Versionen
try:
    from mavsdk.telemetry import ActuatorControlTarget, ActuatorOutputStatus
except ImportError:
    # Diese Klassen sind in neueren Versionen nicht mehr vorhanden
    ActuatorControlTarget = None
    ActuatorOutputStatus = None

try:
    from mavsdk.telemetry import VelocityNed, PositionVelocityNed, GroundTruth
except ImportError:
    # Diese Klassen könnten in einigen Versionen fehlen
    VelocityNed = None
    PositionVelocityNed = None
    GroundTruth = None
...
class MAVSDKConnector(QObject):
    ...
    async def _start_telemetry_subscriptions(self):
        """Startet alle Telemetrie-Subscriptions mit Kompatibilität für verschiedene MAVSDK-Versionen"""
        # Position
        try:
            self._drone.telemetry.position.subscribe(self._handle_position)
            self._logger.addLog("Position-Telemetrie abonniert")
        except (AttributeError, NotImplementedError) as e:
            self._logger.addLog(f"Position-Telemetrie nicht verfügbar: {str(e)}")
        
        # Attitude (Euler)
        try:
            self._drone.telemetry.attitude_euler.subscribe(self._handle_attitude)
            self._logger.addLog("Attitude-Telemetrie abonniert")
        except (AttributeError, NotImplementedError) as e:
            self._logger.addLog(f"Attitude-Telemetrie nicht verfügbar: {str(e)}")
        
        # Battery
        try:
            self._drone.telemetry.battery.subscribe(self._handle_battery)
            self._logger.addLog("Battery-Telemetrie abonniert")
        except (AttributeError, NotImplementedError) as e:
            self._logger.addLog(f"Battery-Telemetrie nicht verfügbar: {str(e)}")
        
        # GPS Info
        try:
            self._drone.telemetry.gps_info.subscribe(self._handle_gps_info)
            self._logger.addLog("GPS-Telemetrie abonniert")
        except (AttributeError, NotImplementedError) as e:
            self._logger.addLog(f"GPS-Telemetrie nicht verfügbar: {str(e)}")
        
        # Status Text
        try:
            self._drone.telemetry.status_text.subscribe(self._handle_status_text)
            self._logger.addLog("Status-Text-Telemetrie abonniert")
        except (AttributeError, NotImplementedError) as e:
            self._logger.addLog(f"Status-Text-Telemetrie nicht verfügbar: {str(e)}")
        
        # Armed Status
        try:
            self._drone.telemetry.armed.subscribe(self._handle_armed)
            self._logger.addLog("Armed-Status-Telemetrie abonniert")
        except (AttributeError, NotImplementedError) as e:
            self._logger.addLog(f"Armed-Status-Telemetrie nicht verfügbar: {str(e)}")
        
        # Flight Mode
        try:
            self._drone.telemetry.flight_mode.subscribe(self._handle_flight_mode)
            self._logger.addLog("Flight-Mode-Telemetrie abonniert")
        except (AttributeError, NotImplementedError) as e:
            self._logger.addLog(f"Flight-Mode-Telemetrie nicht verfügbar: {str(e)}")
        
        # Actuator Control Target - Optional
        if ActuatorControlTarget is not None:
            try:
                self._drone.telemetry.actuator_control_target.subscribe(self._handle_actuator_control_target)
                self._logger.addLog("Actuator-Control-Target-Telemetrie abonniert")
            except (AttributeError, NotImplementedError) as e:
                self._logger.addLog(f"Actuator-Control-Target-Telemetrie nicht verfügbar: {str(e)}")
        
        # Actuator Output Status - Optional
        if ActuatorOutputStatus is not None:
            try:
                self._drone.telemetry.actuator_output_status.subscribe(self._handle_actuator_output_status)
                self._logger.addLog("Actuator-Output-Status-Telemetrie abonniert")
            except (AttributeError, NotImplementedError) as e:
                self._logger.addLog(f"Actuator-Output-Status-Telemetrie nicht verfügbar: {str(e)}")
        
        # Alle Telemetrie-Subscriptions wurden eingerichtet
