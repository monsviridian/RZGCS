#!/usr/bin/env python
"""
Debugging-Skript fu00fcr die RZGCS-Kartenansicht
Dieses Skript testet die Komponenten einzeln, um pru00e4zise Fehlermeldungen zu erhalten
"""
...
def check_environment():
    """Pru00fcft die Umgebungsvariablen und Python-Version"""
    ...
    # Pru00fcfe PySide6.QtWebEngine
    try:
        from PySide6 import QtWebEngineWidgets
        logger.log("QtWebEngineWidgets ist verfu00fcgbar")
    except ImportError as e:
        logger.log(f"QtWebEngineWidgets ist NICHT verfu00fcgbar: {str(e)}")
    
    # Pru00fcfe auf erforderliche Dateien
...
def main():
    """Hauptfunktion fu00fcr den Debug-Prozess"""
    logger.log("=== Starte RZGCS Map Debugging ===\n")
    
    # Pru00fcfe Umgebung
    if not check_environment():
        logger.log("Umgebungspru00fcfung fehlgeschlagen!")
        return
    ...
    # Erstelle Zusammenfassung ohne f-string mit Unicode-Escape-Sequenzen
    check_symbol = "\u2713"  # Häkchen-Symbol
    cross_symbol = "\u2717"  # Kreuz-Symbol
    
    report = []
    report.append("SimpleMapView: " + (check_symbol if simple_map_result else cross_symbol))
    report.append("WebEngine: " + (check_symbol if webengine_result else cross_symbol))
    report.append("FlightMapView: " + (check_symbol if flight_map_result else cross_symbol))
    
    logger.log("Gesamtergebnis: " + " ".join(report))
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Warte kurz, damit die Fenster angezeigt werden ku00f6nnen
        import time
        time.sleep(5)
    except Exception as e:
