"""Backend-KeyEncryptor.

Dieser KeyEncryptor implementiert die Verbindung zwischen Backend-KeySigner und Backend-KeyDecryptor.
"""
...
from flight_control.keydecryptor.backend_keydecryptor import BackendKeyDecryptor

class BackendKeyEncryptor(QObject):
    """Backend-KeyEncryptor.
    
    Dieser KeyEncryptor implementiert die Verbindung zwischen Backend-KeySigner und Backend-KeyDecryptor.
    
    Attributes:
        _backend_keydecryptor: Backend-KeyDecryptor
    ...
    def __init__(self):
        """Initialisierung."""
        super().__init__()
        self._backend_keydecryptor = BackendKeyDecryptor()
        self._frontend_connected = False
        
        # Backend-KeyDecryptor-Signale verbinden
        self._backend_keydecryptor.fleet_changed.connect(self._on_fleet_changed)
        self._backend_keydecryptor.uav_changed.connect(self._on_uav_changed)
        self._backend_keydecryptor.frontend_connected.connect(self._on_frontend_connected)
        self._backend_keydecryptor.frontend_disconnected.connect(self._on_frontend_disconnected)
    ...
    def connect_frontend(self):
        """Frontend verbinden."""
        self._backend_keydecryptor.connect_frontend()
    
    def disconnect_frontend(self):
        """Frontend trennen."""
        self._backend_keydecryptor.disconnect_frontend()
    ...
    def get_fleet_data(self) -> Dict[str, Any]:
        ...
        Returns:
            Flottendaten
        """
        return self._backend_keydecryptor.get_fleet_data()
    ...
    def get_uav_data(self, uav_id: str) -> Dict[str, Any]:
        ...
        Returns:
            UAV-Daten
        """
        return self._backend_keydecryptor.get_uav_data(uav_id)
    ...
    def update_fleet_data(self, fleet_data: Dict[str, Any]):
        ...
        """
        self._backend_keydecryptor.update_fleet_data(fleet_data)
    ...
    def update_uav_data(self, uav_id: str, uav_data: Dict[str, Any]):
        ...
        Args:
            ...
            uav_data: UAV-Daten
        """
        self._backend_keydecryptor.update_uav_data(uav_id, uav_data)
