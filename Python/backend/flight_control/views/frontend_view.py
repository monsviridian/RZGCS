...
Diese View implementiert die Benutzeroberfläche für das Frontend.
"""
...
class FrontendView(QWidget):
    ...
    def set_viewmodel(self, viewmodel: FrontendViewModel):
        """ViewModel setzen.
        
        Args:
            viewmodel: Frontend-ViewModel
        """
        ...
        self._viewmodel.frontend_connected.connect(self._on_frontend_connected)
        self._viewmodel.frontend_disconnected.connect(self._on_frontend_disconnected)
    ...
    def _on_frontend_connected(self):
        """Frontend verbunden."""
        self._fleet_button.setEnabled(True)
        self._uav_button.setEnabled(True)
        self._resources_button.setEnabled(True)
        self._communication_button.setEnabled(True)
        self._coordination_button.setEnabled(True)
        self._collision_button.setEnabled(True)
    
    def _on_frontend_disconnected(self):
        """Frontend getrennt."""
        self._fleet_button.setEnabled(False)
        self._uav_button.setEnabled(False)
        self._resources_button.setEnabled(False)
        self._communication_button.setEnabled(False)
        self._coordination_button.setEnabled(False)
        self._collision_button.setEnabled(False)
    ...
    def _on_fleet_mode_changed(self, mode: str):
        ...
        try:
            self._viewmodel.update_fleet_data({
    ...
    def _on_uav_clicked(self):
        ...
        try:
            self._viewmodel.update_uav_data(f"uav_{len(self._viewmodel.uavs) + 1}", {
