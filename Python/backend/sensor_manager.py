...
import time
import re
# SensorManager implementiert selbst ViewModel-Funktionalität
...
class SensorManager(QObject):
    ...
    sensorListChanged = Signal()  # QML-Integration: Signal für Aktualisierung der Sensorliste
    ...
    def __init__(self, logger: Logger):
        ...
        # Sensordaten direkt im SensorManager speichern
        self._sensors = {}
        self._sensor_list = []  # Für QML Liste
    ...
    def handle_attitude(self, msg):
        ...
        try:
            ...
            self.update_sensor("roll", roll)
            self.update_sensor("pitch", pitch)
            self.update_sensor("yaw", yaw)
    ...
    def handle_gps(self, msg):
        ...
        try:
            ...
            # Speichere in Python-Modell
            self.update_sensor("gps_lat", round(lat, 6))
            self.update_sensor("gps_lon", round(lon, 6))
            self.update_sensor("altitude", round(alt, 1))
            
            # Calculate ground speed
            vx = msg.vx / 100.0
            vy = msg.vy / 100.0
            ground_speed = (vx*vx + vy*vy)**0.5
            self.update_sensor("groundspeed", round(ground_speed, 1))
    ...
    def _reset_sensor_values(self):
        ...
        self._update_qml_sensor("System Servos", "Nicht verbunden", "")
        self._update_qml_sensor("System RC", "Nicht verbunden", "")
        self._update_qml_sensor("System Mission", "Nicht verbunden", "")
        self._update_qml_sensor("System CPU", "Nicht verbunden", "")
        self._update_qml_sensor("Battery %", "Nicht verbunden", "")
    ...
    def handle_battery(self, msg):
        ...
        try:
            ...
            if hasattr(msg, 'load'):
                ...
                self.updateQmlSensor("CPU Last", round(cpu_load, 1), "%")
                # Formatiere CPU-Last für SensorView 
                formatted_cpu = f"{round(cpu_load, 1)}%"
                self.updateQmlSensor("System CPU", formatted_cpu, "")
            
            # Speichere in Python-Modell
            self.update_sensor("battery_voltage", round(voltage, 1))
            self.update_sensor("battery_current", round(current, 1))
            self.update_sensor("battery_remaining", round(remaining, 0))
            ...
            # Direkt zum QML-Modell senden ohne Ratenbegrenzung zum Testen
            self.updateQmlSensor("Battery", battery_data, "V")
            # Zeige den Batterieprozentsatz auch als eigenen Sensor an
            self.updateQmlSensor("Battery %", f"{display_remaining}%", "")
    ...
    def handle_vfr_hud(self, msg):
        ...
        try:
            ...
            # Speichere in Python-Modell
            self.update_sensor("airspeed", round(airspeed, 1))
            self.update_sensor("groundspeed", round(groundspeed, 1))
            self.update_sensor("heading", round(heading, 1))
            self.update_sensor("throttle", round(throttle, 0))
            if alt > 0:
                self.update_sensor("altitude", round(alt, 1))
            self.update_sensor("climb", round(climb, 1))
    ...
    def handle_servo_output(self, msg):
        ...
        try:
            ...
            if servo_values:
                ...
                self.updateQmlSensor("System Servos", servo_info, "")
        except Exception as e:
    ...
    def handle_rc_channels(self, msg):
        ...
        try:
            ...
            if rc_values:
                ...
                # Aktualisiere direkt im QML-Modell ohne Ratenbegrenzung zum Testen
                self.updateQmlSensor("System RC", rc_info, "")
        except Exception as e:
    ...
    def handle_mission(self, msg):
        ...
        try:
            ...
            if hasattr(msg, 'seq'):
                ...
                # Direkt zum QML-Modell senden
                self.updateQmlSensor("System Mission", mission_info, "")
                
                # Speichere auch die Wegpunkt-Nummer als Sensor
                self.update_sensor("mission_waypoint", waypoint)
    ...
    def _update_qml_sensor(self, name, value, unit):
        ...
        try:
            ...
            # Debug-Log für Sensor-Updates
            self._logger.addLog(f"DEBUG: Sensor-Update {name}={value} {unit}")
            ...
            # Sensor im Dictionary aktualisieren
            sensor_data = {"value": value, "unit": unit}
            self._sensors[name] = sensor_data
            
            # Emit signal für externe Observer
            self.sensorUpdated.emit(name, value)
        except Exception as e:
            self._logger.addLog(f"Error updating QML sensor: {str(e)}")
        
    @Slot(str, str, str)
    def updateQmlSensor(self, name, value, unit):
        """Public interface for updating a sensor value from QML or external components"""
        try:
            # Direkte Weiterleitung an die interne Methode
            self._update_qml_sensor(name, value, unit)
        except Exception as e:
            self._logger.addLog(f"Error in updateQmlSensor: {str(e)}")
    ...
    def initialize_sensors(self):
        ...
        for sensor_id, name, unit in sensors:
            try:
                self.add_sensor(sensor_id, name, unit)
                self.update_sensor(sensor_id, 0.0)
                self._logger.addLog(f"Sensor initialisiert: {name}")
            except Exception as e:
