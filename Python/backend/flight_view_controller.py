...
class FlightViewController(QObject):
    ...
    # Signale für die Drohnenposition und -ausrichtung (für die CoDMinimap)
    ...
    def __init__(self, engine, parent=None):
        ...
        # Drone status - Frankfurt coordinates
        self._drone_lat = 50.110924
        self._drone_lon = 8.682127
        self._drone_alt = 100.0
        self._drone_heading = 45.0
    ...
    def initialize(self, root_item):
        ...
        try:
            print("[INFO] Initializing FlightViewController...")
            
            # Find FlightView container
            flight_view = root_item.findChild(QObject, "flightView")
            if not flight_view:
                print("[ERROR] FlightView not found in QML")
                return False

            print("[INFO] FlightView found")
            
            # Connect signals - nur einmal verbinden
            ...
            # Connect to serialConnector signal (if available in the context)
            # This will help us track the connection status
            serial_connector = self.engine.rootContext().contextProperty("serialConnector")
            if serial_connector:
                # Versuche zuerst mit der Standard-Eigenschaft 'connected'
                try:
                    # BUGFIX: Benutze die korrekte Signal-Verbindungssyntax
                    # Das Problem: Bei falscher Typisierung wie serial_connector.connectedChanged[bool].connect(...)
                    # kommt es zu dem Fehler "PySide6.QtCore.QObject.connect(): not enough arguments"
                    
                    if hasattr(serial_connector, 'connectedChanged'):
                        # Versuche direkten Connection zu einer Methode statt zum Signal
                        try:
                            # Rufe connect ohne Parameter auf, um die Verbindung herzustellen
                            # und verwende dann den bereits gespeicherten Port
                            serial_connector.connect()
                            print("[INFO] Successfully called serialConnector.connect() directly")
                            
                            # Nutze die property als Fallback für die Statuserkennung
                            if hasattr(serial_connector, 'connected'):
                                connected = serial_connector.property('connected')
                                self.on_connection_changed(bool(connected))
                                print(f"[INFO] Connection status manually checked: {connected}")
                        except Exception as conn_err:
                            print(f"[WARNING] Could not call connect method: {str(conn_err)}")
                    else:
                        print("[WARNING] connectedChanged signal not available on serialConnector")
                except Exception as e:
                    print(f"[WARNING] Could not connect to connectedChanged signal: {str(e)}")
                    
                # Versuche mit dem connection_adapter-Signal status_changed als Fallback
                try:
                    from backend.connection.connection_types import ConnectionStatus
                    connection_adapter = self.engine.rootContext().contextProperty("connection_adapter")
                    if connection_adapter and hasattr(connection_adapter, 'status_changed'):
                        # Direkte Verbindung mit dem korrekten Enum-Typ
                        connection_adapter.status_changed.connect(
                            lambda status: self.on_connection_changed(status == ConnectionStatus.CONNECTED)
                        )
                        print("[INFO] Successfully connected to connection_adapter.status_changed signal")
                    else:
                        print("[WARNING] connection_adapter not found in QML context or has no status_changed signal")
                except Exception as e2:
                    print(f"[ERROR] Failed to connect to connection signals: {str(e2)}")
            else:
                print("[WARNING] serialConnector not found in QML context")
            ...
            # Starte Simulation nur, wenn verbunden
            if self._is_connected:
                print("[INFO] Starting simulation timer")
                self.sim_timer.start()
            else:
                print("[INFO] Simulation paused until connection is established")
    ...
    def on_connection_changed(self, connected):
        """Wird aufgerufen, wenn sich der Verbindungsstatus u00e4ndert"""
        print(f"[INFO] Connection status changed: {connected}")
        self._is_connected = connected
        
        # Starte oder stoppe die Simulation basierend auf dem Verbindungsstatus
        if connected:
            if not self.sim_timer.isActive():
                print("[INFO] Starting simulation timer due to connection")
                self.sim_timer.start()
        else:
            if self.sim_timer.isActive():
                print("[INFO] Stopping simulation timer due to disconnection")
                self.sim_timer.stop()
            # Reset values to Frankfurt coordinates when disconnected
            self._drone_lat = 50.110924
            self._drone_lon = 8.682127
            self._drone_alt = 100.0
            self._drone_heading = 45.0
            # Send reset values to UI
    ...
    def simulate_drone_movement(self):
        ...
        # Wenn nicht verbunden, nichts tun
    ...
    def update_drone_position(self, lat, lon, alt, heading=0, speed=0, battery=100):
        ...
        self.dronePositionChanged.emit(lat, lon, alt)
        self.droneHeadingChanged.emit(heading)
