    ...
    ColumnLayout {
        ...
        RowLayout {
            ...
            Button {
                ...
                onClicked: {
                    let connString = ""
                    if (connectionType.currentText === "Simulator") {
                        connString = "simulator://"
                    } else if (connectionType.currentText === "Serial") {
                        const baudrate = connectionString.text.trim() || "115200"
                        connString = `${portCombo.currentText}:${baudrate}`
                    } else {
                        connString = connectionString.text.trim()
                    }
                    
                    if (connString) {
                        serialConnector.connectDrone(connString)
                    }
                }
            }
